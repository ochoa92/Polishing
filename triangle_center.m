function [C] = triangle_center(Pa,Pb,Pc,Type)
    ...
    %
    % Example: triangle_center([0 0.5 0], [1 0 0], [1 1 3], 'orthocenter')
    %
    ...
    C = Pa + lambda(1)*L1; % Line Equation evaluated at lambda(1)
%     X = [Pa(1); Pb(1); Pc(1)]; 
%     Y = [Pa(2); Pb(2); Pc(2)]; 
...
%         hold on;
%         plot3(C(1),C(2),C(3),'o','Color','r'); 
%         view(3);
%     else             % Bidimensional case
%         patch(X,Y,'b','FaceAlpha',0.5); 
%         hold on;
%         plot(C(1),C(2),'o','Color','r');
%     end
%     grid on; 
%     daspect([1 1 1]); 
end